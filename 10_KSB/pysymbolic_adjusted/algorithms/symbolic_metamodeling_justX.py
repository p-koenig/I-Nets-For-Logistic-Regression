...
class symbolic_metamodel:
    ...
    def __init__(self, model, X, mode="classification", approximation_order=3, force_polynomial=False, verbosity=False, early_stopping=None, restarts=0):
        ...
        self.num_basis        = self.X.shape[1]#self.X_new.shape[1]
    ...
    def set_equation(self, reset_init_model=False):
        ...
        self.X_init           = compose_features(self.params, self.X, approximation_order=self.approximation_order)#compose_features(self.params, self.X_new, approximation_order=self.approximation_order)
    ...
    def get_gradients(self, Y_true, Y_metamodel, batch_index=None):
        ...
        for k in range(self.params.shape[0]):
            ...
            if batch_index is None:
                grads_vals    = basis_grad(a, b, c, self.X[:, k] +  epsilon)#basis_grad(a, b, c, self.X_new[:, k] +  epsilon)
            else:
                grads_vals    = basis_grad(a, b, c, self.X[batch_index, k] +  epsilon)#basis_grad(a, b, c, self.X_new[batch_index, k] +  epsilon)
    ...
    def fit(self, num_iter=10, batch_size=100, learning_rate=.01):
        ...
        for i in self.tqdm_mode(range(num_iter)):
            batch_index = np.random.choice(list(range(self.X.shape[0])), size=batch_size)#np.random.choice(list(range(self.X_new.shape[0])), size=batch_size)
            
            #print('batch_index', batch_index[:10])
                        
            if np.isnan(self.X[batch_index, :]).any() or np.isinf(self.X[batch_index, :]).any():#if np.isnan(self.X_init[batch_index, :]).any() or np.isinf(self.X_init[batch_index, :]).any():
                print('\n\nBREAK X_init', i)
                ...
                if self.verbosity:
                    print('\n\nBREAK X_init')
                    print('self.X[batch_index, :]', self.X[batch_index, :])#print('self.X_init[batch_index, :]', self.X_init[batch_index, :])
            ...
            print('self.init_model', self.init_model)
            print('self.init_model.coef_', self.init_model.coef_)
            curr_func   = self.init_model.predict(np.nan_to_num(self.X_init[batch_index, :]))
            print('curr_func INIT MODEL', np.round(curr_func, 3)[:5])
                    
            
            #evaluater_  = lambdify([self.approx_expression.atoms(Symbol)], self.approx_expression, modules=['math'])
            #evaluater_  = lambdify([self.exact_expression.atoms(Symbol)], self.exact_expression, modules=['math'])
            #curr_func           = np.real(np.array([evaluater_(np.nan_to_num(self.X_init[batch_index, :])[k]) for k in range(len(np.nan_to_num(self.X_init[batch_index, :])))]))
            
            curr_func   = calculate_function_values_from_sympy(self.exact_expression, np.nan_to_num(self.X[batch_index, :]))#calculate_function_values_from_sympy(self.exact_expression, np.nan_to_num(self.X_init[batch_index, :]))
            ...
            print('optimizer_params START')
            print('param_grads', param_grads)
            print('params_variable', params_variable)   
            print('list([list(zip(grads, variables)) for grads, variables in zip(param_grads, params_variable)])', list([list(zip(grads, variables)) for grads, variables in zip(param_grads, params_variable)])) 
            print('flatten_list(list([list(zip(grads, variables)) for grads, variables in zip(param_grads, params_variable)]))', flatten_list(list([list(zip(grads, variables)) for grads, variables in zip(param_grads, params_variable)])))           
            ...
            coef_grads            = [self.loss_grad_coeff(self.Y_r[batch_index], curr_func, self.X[batch_index, k]) for k in range(self.X.shape[1])]#[self.loss_grad_coeff(self.Y_r[batch_index], curr_func, self.X_init[batch_index, k]) for k in range(self.X_init.shape[1])]
            print('optimizer_coef_ START')
            print('coef_grads', coef_grads)
            print('coef_variable', coef_variable)
            print('zip(coef_grads, coef_variable)', list(zip(coef_grads, coef_variable)))
            optimizer_coef_.apply_gradients(zip(coef_grads, coef_variable))
            print('optimizer_coef_ END')
    ...
    def evaluate(self, X):
        ...
        X_modified  = X#self.feature_expander.fit_transform(X)
