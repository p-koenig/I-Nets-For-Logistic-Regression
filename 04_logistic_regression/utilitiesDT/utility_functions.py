...
#udf import
from utilitiesDT.LambdaNet import *
from utilitiesDT.metrics import *
from utilitiesDT.DecisionTree_BASIC import *
from utilitiesDT.InterpretationNet import *

#from utilitiesDT.utility_functions import *
...
from utilitiesDT.make_classification_distribution import make_classification_distribution
...
def data_path(config):
    return  'dataDT/nda' + str(config['data']['n_datasets']) + '_nsa' + str(config['data']['n_samples']) + '_nfe' + str(config['data']['n_features']) + '_nin' + str(config['data']['n_informative']) + '_nta' + str(config['data']['n_targets']) + '_bia' + str(config['data']['bias']) + '_eff' + str(config['data']['effective_rank']) + '_tai' + str(config['data']['tail_strength']) + '_noi' + str(config['data']['noise']) + '_shu' + str(config['data']['shuffle']) + '_ran' + str(config['data']['random_state']) 

def lambda_path(config, datasetIndex):
    return  data_path(config) + '/' + str(datasetIndex) + '_tsi' + str(config['lambda']['data_prep']['train_test_val_split']['test_size']) + '_vsi' + str(config['lambda']['data_prep']['train_test_val_split']['val_size']) + '_ran' + str(config['lambda']['data_prep']['train_test_val_split']['random_state']) + '_shu' + str(config['lambda']['data_prep']['train_test_val_split']['shuffle']) + '_str' + str(config['lambda']['data_prep']['train_test_val_split']['stratify']) + '_bat' + str(config['lambda']['model_fit']['batch_size']) + '_epo' + str(config['lambda']['model_fit']['epochs']) + '_shu' +  str(config['lambda']['model_fit']['shuffle']) + '_cla' + str(config['lambda']['model_fit']['class_weight']) + '_sam' + str(config['lambda']['model_fit']['sample_weight']) + '_ini' + str(config['lambda']['model_fit']['initial_epoch']) + '_ste' + str(config['lambda']['model_fit']['steps_per_epoch']) + '_vst' + str(config['lambda']['model_fit']['validation_steps']) + '_vbs' + str(config['lambda']['model_fit']['validation_batch_size']) + '_vfr' + str(config['lambda']['model_fit']['validation_freq'])


def inet_general_path(config):
    return data_path(config) + '/inets/' 

def inet_path(config):
    ...
    path_identifier_interpretation_net = (inet_general_path(config) +
                                               '/' +
                                               'inet' + interpretation_net_identifier)
    
    return path_identifier_interpretation_net
...
def return_callbacks_from_string(callback_string_list, config=None):
    ...
    from utilitiesDT.InterpretationNet import CustomStopper
    ...
    if 'tensorboard' in callback_string_list:
        ...
        log_dir = './dataDT/logging/' + paths_dict['path_identifier_interpretation_net'] + '/tensorboard'+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        #log_dir = './dataDT/logging/' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
def generate_paths(config, path_type='interpretation_net'):
    ...
    noise = '' if config['data']['noise'] is None else '_dNoise' + str(config['data']['noise'])
                
    decision_sparsity = -1 if decision_sparsity == config['data']['n_features'] else decision_sparsity
    ...
    data_specification_string = (
                                  '_var' + str(config['data']['n_features']) +
                                  '_class' + str(config['data']['n_targets']) +
    ...
    if path_type == 'data_creation' or path_type == 'lambda_net' or path_type == 'interpretation_net': #Data Generation
        ...
        path_identifier_function_data = ('lNetSize' + str(config['data']['n_samples']) +
                                         '_numDatasets' + str(config['data']['n_datasets']) +
                                         data_specification_string)            
    ...
    if path_type == 'lambda_net' or path_type == 'interpretation_net': #Lambda-Net
        ...
        lambda_noise_string = '_noise-' + config['data']['noise_injected_type'] + str(config['data']['noise']) if config['data']['noise'] > 0 else ''
        ...
        path_identifier_lambda_net_data = ('lNetSize' + str(config['data']['n_samples']) +
    ...
    if path_type == 'interpretation_net': #Interpretation-Net   
        ...
        path_identifier_interpretation_net = ('INetSize' + str(config['data']['n_samples']) +
                                                   '_numLNets' + str(config['lambda_net']['number_of_trained_lambda_nets']) +
                                                   data_specification_string + 
...
def create_folders_inet(config):
    ...
    try:
        ...
        os.makedirs('./dataDT/logging/' + paths_dict['path_identifier_interpretation_net'] + '/')
        os.makedirs('./dataDT/plotting/' + paths_dict['path_identifier_interpretation_net'] + '/')
        os.makedirs('./dataDT/results/' + paths_dict['path_identifier_interpretation_net'] + '/')
...
def generate_directory_structure():
    ...
    if not os.path.exists('./data'):
        ...
        text_file = open('./dataDT/.gitignore', 'w')
        text_file.write('*')
        text_file.close()  
        
    for directory_name in directory_names:
        path = './dataDT/' + directory_name
...
def generate_lambda_net_directory(config):
    ...
    try:
        # Create target Directory
        os.makedirs('./dataDT/weights/weights_' + paths_dict['path_identifier_lambda_net_data'])

    except FileExistsError:
        folder = './dataDT/weights/weights_' + paths_dict['path_identifier_lambda_net_data']
    ...
    try:
        # Create target Directory
        os.makedirs('./dataDT/results/weights_' + paths_dict['path_identifier_lambda_net_data'])
    except FileExistsError:
        pass
...
def get_parameters_from_sklearn_decision_tree(tree, config, printing=False):
    ...
    features = [i for i in range(config['data']['n_features'])]
...
def get_shaped_parameters_for_decision_tree(flat_parameters, config, eager_execution=False):
    ...
    output_dim = config['data']['n_targets']
    ...
    if config['i_net']['function_representation_type'] == 1:
        if config['function_family']['dt_type'] == 'SDT':
            ...
            if config['function_family']['decision_sparsity'] != -1 and config['function_family']['decision_sparsity'] !=  config['data']['n_features']:
            ...
            splits_index = tf.cast(tf.clip_by_value(tf.round(flat_parameters[config['function_family']['decision_sparsity']*internal_node_num_:(config['function_family']['decision_sparsity']*internal_node_num_)*2]), clip_value_min=0, clip_value_max=config['data']['n_features']-1), tf.int64)
        ...
        if config['function_family']['dt_type'] == 'SDT':
            ...
            weights_index_array = flat_parameters[internal_node_num_ * config['function_family']['decision_sparsity']:(internal_node_num_ * config['function_family']['decision_sparsity'])+(internal_node_num_ * config['function_family']['decision_sparsity'] * config['data']['n_features'])]
            
            biases = flat_parameters[(internal_node_num_ * config['function_family']['decision_sparsity'])+(internal_node_num_ * config['function_family']['decision_sparsity'] * config['data']['n_features']):(internal_node_num_ * config['function_family']['decision_sparsity'])+(internal_node_num_ * config['function_family']['decision_sparsity'] * config['data']['n_features']) + internal_node_num_]
            
            leaf_probabilities = flat_parameters[(internal_node_num_ * config['function_family']['decision_sparsity'])+(internal_node_num_ * config['function_family']['decision_sparsity'] * config['data']['n_features']) + internal_node_num_:]
            leaf_probabilities = tf.transpose(tf.reshape(leaf_probabilities, (leaf_node_num_, output_dim)))
            ...
            if False: #duplicates in predicted index not considered/summarized
                ...
                for indices_node, values_node in zip(weights_index_list_by_internal_node_by_decision_sparsity_argmax,  weights_coeff_list_by_internal_node):
                    if False:
                        dense_tensor = []
                        for i in range(config['data']['n_features']):
                        ...
                        dense_tensor = []#[0 for _ in range(config['data']['n_features'])]
                        for i in range(config['data']['n_features']):
                            index_identifier = []
            ...
            split_index_num_params = config['data']['n_features'] *  config['function_family']['decision_sparsity'] * internal_node_num_
            leaf_classes_num_params = leaf_node_num_ #* config['data']['n_targets']
        ...
        if config['function_family']['dt_type'] == 'SDT':    
            ...
            split_values_num_params = config['data']['n_features'] * internal_node_num_#config['function_family']['decision_sparsity']
            split_index_num_params = config['data']['n_features'] * internal_node_num_
            leaf_classes_num_params = leaf_node_num_ #* config['data']['n_targets']
...
def generate_decision_tree_from_array(parameter_array, config):
    ...
    if config['function_family']['dt_type'] == 'SDT': 
        ...
        tree = SDT(input_dim=config['data']['n_features'],
                   output_dim=config['data']['n_targets'],
...
def generate_random_data_points(config, seed, parameters=None):
    ...
    size = config['data']['n_samples'] 
    variables = config['data']['n_features'] 
...
def generate_random_decision_tree(config, seed=42, verbosity=0):
    ...
    if config['function_family']['dt_type'] == 'SDT':    
        if config['function_family']['fully_grown']:
            tree = SDT(input_dim=config['data']['n_features'],#X_train.shape[1], 
                       output_dim=config['data']['n_targets'],#int(max(y_train))+1, 
                       depth=config['function_family']['maximum_depth'],
                       beta=config['function_family']['beta'],
                       decision_sparsity=config['function_family']['decision_sparsity'],
                       random_seed=seed,
...
def generate_data_random_decision_tree(config, seed=42):
    ...
    if config['function_family']['dt_type'] == 'SDT':    
        ...
        while np.unique(np.round(y_data)).shape[0] == 1 or np.min(np.unique(np.round(y_data), return_counts=True)[1]) < config['data']['n_samples']/4:
            seed = seed+(config['data']['n_datasets'] * counter)    
            counter += 1
        ...
        for _ in range(internal_node_num_):
            random_number = np.random.randint(0, config['data']['n_features'])
...
def generate_data_random_decision_tree_trained(config, seed=42):
    ...
    if config['function_family']['dt_type'] == 'SDT':    
        ...
        #placeholder = [0 for i in range((2 ** config['function_family']['maximum_depth'] - 1) * config['data']['n_features'] + (2 ** config['function_family']['maximum_depth'] - 1) + (2 ** config['function_family']['maximum_depth']) * config['data']['n_targets'])]
...
def generate_data_make_classification_decision_tree_trained(config, seed=42):
    ...
    informative = config['data']['n_features']#np.random.randint(config['data']['n_features']//2, high=config['data']['n_features']+1) #config['data']['n_features']
    redundant = 0#np.random.randint(0, high=config['data']['n_features']-informative+1) #0
    repeated = 0#config['data']['n_features']-informative-redundant # 0

    n_clusters_per_class =  max(2, np.random.randint(0, high=informative//2+1)) #2

    X_data, y_data_tree = make_classification(n_samples=config['data']['n_samples'], 
                                                       n_features=config['data']['n_features'], #The total number of features. These comprise n_informative informative features, n_redundant redundant features, n_repeated duplicated features and n_features-n_informative-n_redundant-n_repeated useless features drawn at random.
                                                       n_informative=informative,#config['data']['n_features'], #The number of informative features. Each class is composed of a number of gaussian clusters each located around the vertices of a hypercube in a subspace of dimension n_informative.
...
def generate_data_make_classification(config, seed=42):
    ...
    X_data, y_data = make_classification(n_samples=config['data']['n_samples'], 
                                                       n_features=config['data']['n_features'], #The total number of features. These comprise n_informative informative features, n_redundant redundant features, n_repeated duplicated features and n_features-n_informative-n_redundant-n_repeated useless features drawn at random.
                                                       n_informative=informative,#config['data']['n_features'], #The number of informative features. Each class is composed of a number of gaussian clusters each located around the vertices of a hypercube in a subspace of dimension n_informative.
                                                       n_redundant=redundant, #The number of redundant features. These features are generated as random linear combinations of the informative features.
                                                       n_repeated=repeated, #The number of duplicated features, drawn randomly from the informative and the redundant features.
    ...
    function_representation_length = ( 
       ((2 ** config['function_family']['maximum_depth'] - 1) * config['data']['n_features']) + (2 ** config['function_family']['maximum_depth'] - 1) + (2 ** config['function_family']['maximum_depth']) * config['data']['n_targets']
  if config['function_family']['dt_type'] == 'SDT'
...
def generate_data_make_classification_distribution_decision_tree_trained(config, seed=42):
    ...
    np.random.seed(seed)
    informative = 3#np.random.randint(config['data']['n_features']//2, high=config['data']['n_features']+1) #config['data']['n_features']
    redundant = np.random.randint(0, high=config['data']['n_features']-informative+1) #0
    repeated = config['data']['n_features']-informative-redundant # 0
    ...
    X_data, y_data, distribution_parameter_list  = make_classification_distribution(n_samples=config['data']['n_samples'], 
                                                       n_features=config['data']['n_features'], #The total number of features. These comprise n_informative informative features, n_redundant redundant features, n_repeated duplicated features and n_features-n_informative-n_redundant-n_repeated useless features drawn at random.
                                                       n_informative=informative,#config['data']['n_features'], #The number of informative features. Each class is composed of a number of gaussian clusters each located around the vertices of a hypercube in a subspace of dimension n_informative.
                                                       n_redundant=redundant, #The number of redundant features. These features are generated as random linear combinations of the informative features.
                                                       n_repeated=repeated, #The number of duplicated features, drawn randomly from the informative and the redundant features.
    ...
    if config['function_family']['dt_type'] == 'SDT':   
        decision_tree.fit(X_data, y_data, epochs=50)    
...
def generate_data_distribution_trained(config, 
    ...
    X_data, y_data_tree, distribution_parameter_list, _ = generate_dataset_from_distributions(distribution_list = config['data']['distribution_list'], 
                                                               ...
                                                               number_of_samples = config['data']['n_samples'], 
                                                               distributions_per_class = distributions_per_class, 
                                                               seed = seed, 
                                                               noise = noise,
...
def anytree_decision_tree_from_parameters(dt_parameter_array, config, normalizer_list=None, path='./dataDT/plotting/temp.png'):
...
def generate_decision_tree_identifier(config):
    ...
    if config['function_family']['dt_type'] == 'SDT':
        ...
        filter_shape = (num_internal_nodes, config['data']['n_features'])
        bias_shape = (num_internal_nodes, 1)

        leaf_probabilities_shape = (num_leaf_nodes, config['data']['n_targets'])
...
#def dt_array_to_sklearn(vanilla_dt_array, config, printing=False):
    ...
    #y_data = [i for i in range(config['data']['n_targets'])]
    #X_data = [[0 for i in range(config['data']['n_features'])] for _ in range(config['data']['n_targets'])]
...
def get_number_of_function_parameters(dt_type, maximum_depth, n_features, number_of_classes):
    
    number_of_function_parameters = None
    
    if dt_type == 'SDT':
        number_of_function_parameters = (2 ** maximum_depth - 1) * (n_features + 1) + (2 ** maximum_depth) * number_of_classes
    ...
    if distribution_dict_list is not None:
        ...
        for i in range(n_features):
            ...
            if config['data']['fixed_class_probability']:
                ...
                class_0_distrib_ratio = distribution_dict_list[i][distribution_name]['samples_class_0']/config['data']['n_samples']
        ...
        X_data = X_data + X_data * np.random.uniform(-noise, noise, X_data.shape) #np.random.uniform(-flip_percentage, flip_percentage, X_data.shape)#         
            ...
            if distributions_per_class == 0:
                ...
                for i in range(n_features):   
                    ...
                    for j in range(distributions_per_class):
                        ...
                        if not config['data']['shift_distrib']:#condition or config['data']['n_datasets'] == 11111:
                            if True:
...
def distribution_evaluation_single_model_synthetic_data(loss_function, 
    ...
    from utilitiesDT.InterpretationNet import load_inet
...
def evaluate_real_world_dataset(model,
    ...
    if not config['i_net']['force_evaluate_real_world'] and X_data.shape[1] != config['data']['n_features']:
    ...
    X_data = adjust_data_to_n_features(X_data=X_data, 
                                                y_data=y_data, 
                                                n_features=config['data']['n_features'],
                                                seed=config['computation']['RANDOM_SEED'])
    X_data, normalizer_list = normalize_real_world_data(X_data)
...
def evaluate_interpretation_net_prediction_single_sample(lambda_net_parameters_array, 
    ...
    from utilitiesDT.metrics import calculate_function_value_from_decision_tree_parameters_wrapper, calculate_function_value_from_vanilla_decision_tree_parameters_wrapper
...
def evaluate_interpretation_net_synthetic_data(network_parameters_array,
    ...
    from utilitiesDT.InterpretationNet import autoencode_data, load_encoder_model, restructure_data_cnn_lstm
...
def adjust_data_to_n_features(X_data, y_data, n_features, seed=42):

    if X_data.shape[1] > n_features:
        #X_data = X_data.sample(n=n_features,axis='columns')
        ...
        for i in range(n_features-X_data.shape[1]):
...
def train_network_real_world_data(X_train, y_train, X_valid, y_valid, config, verbose=1):
    ...
    from utilitiesDT.LambdaNet import generate_lambda_net_from_config
...
def split_LambdaNetDataset(dataset, test_split, random_seed=42):
    ...
    from utilitiesDT.LambdaNet import LambdaNetDataset
...
def generate_base_model(config, disable_batchnorm = False): #without dropout
    ...
    output_neurons = 1 if config['data']['n_targets']==2 else config['data']['n_targets']
    output_activation = 'sigmoid' if config['data']['n_targets']==2 else 'softmax'
    ...
    model.add(Dense(config['lambda_net']['lambda_network_layers'][0], activation='relu', input_dim=config['data']['n_features'], kernel_initializer=tf.keras.initializers.GlorotUniform(seed=config['computation']['RANDOM_SEED']), bias_initializer='zeros'))
...
def shape_flat_network_parameters(flat_network_parameters, target_network_parameters):
    ...
    #from utilitiesDT.utility_functions import flatten_list
...
def per_network_dt_optimization_tf(network_parameters,
    ...
    from utilitiesDT.metrics import calculate_function_value_from_lambda_net_parameters_wrapper, calculate_function_value_from_decision_tree_parameters_wrapper, calculate_function_value_from_vanilla_decision_tree_parameters_wrapper
    ...
    #random_evaluation_dataset =  np.random.uniform(low=0, high=0.2, size=(config['evaluation']['random_evaluation_dataset_size'], config['data']['n_features']))
    np.random.seed(config['computation']['RANDOM_SEED'])
    random_evaluation_dataset =  np.random.uniform(low=config['data']['x_min'], high=config['data']['x_max'], size=(config['evaluation']['per_network_dataset_size'], config['data']['n_features']))
...
def generate_dataset_from_distributions_line(line_distribution_parameters,
    ...
    try:
        if config['data']['max_distributions_per_class'] != 0:
            samples_class_0_list = line_distribution_parameters[:config['data']['n_features']*3].reshape(-1, 3).T[0]
            samples_class_0_list = np.array(samples_class_0_list).astype(np.int64)

            feature_weight_0_list = line_distribution_parameters[:config['data']['n_features']*3].reshape(-1, 3).T[1]
            feature_weight_0_list = np.array(feature_weight_0_list).astype(np.float32)

            seed_shuffeling_list = line_distribution_parameters[:config['data']['n_features']*3].reshape(-1, 3).T[2]
            seed_shuffeling_list = np.array(seed_shuffeling_list).astype(np.int64)

            line_distribution_parameters = line_distribution_parameters[config['data']['n_features']*3:]
        else:
            samples_class_0_list = [np.nan]* config['data']['n_features']
            feature_weight_0_list = [np.nan]* config['data']['n_features']
            seed_shuffeling_list = [np.nan]* config['data']['n_features']
    except:
        samples_class_0_list = [np.nan]* config['data']['n_features']
        feature_weight_0_list = [np.nan]* config['data']['n_features']
        seed_shuffeling_list = [np.nan]* config['data']['n_features']
    ...
    if config['data']['max_distributions_per_class'] == 0:
        ...
        distribution_list = line_distribution_parameters.reshape(-1, 1+max_distributions_per_class_function*config['data']['n_targets']*2)    
...
def plot_decision_area_evaluation(X_train, 
    ...
    if config['function_family']['dt_type'] == 'vanilla':
        ...
        tree_inet = SDT(input_dim=config['data']['n_features'],
                           output_dim=config['data']['n_targets'],
                           depth=config['function_family']['maximum_depth'],
                           beta=config['function_family']['beta'],
                           decision_sparsity=config['function_family']['decision_sparsity'],
...
def plot_decision_area_evaluation_all_distrib(X_train, 
    ...
    if identifier_folder is not None and config['data']['n_features'] == 2:
        paths_dict = generate_paths(config, path_type = 'interpretation_net')
    
        plt.savefig('./dataDT/distrib_plots/' + identifier_folder + '/' + identifier_file + '.pdf', bbox_inches = 'tight', pad_inches = 0)
...
def evaluate_network_on_distribution_custom_parameters(distribution_name_feature_0,
    ...
    if config['data']['n_features']  == 2:
        ...
        X_data, y_data, distribution_parameter_list, normalizer_list = generate_dataset_from_distributions(distribution_list = None, 
                                                                                                        ...
                                                                                                        distributions_per_class = config['data']['max_distributions_per_class'], 
                                                                                                        seed = config['computation']['RANDOM_SEED'], 
                                                                                                        flip_percentage = config['data']['noise'],  
