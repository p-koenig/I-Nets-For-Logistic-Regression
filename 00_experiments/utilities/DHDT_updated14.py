...
from utilities.utilities_updated14 import *
from utilities.DHDT_updated14 import *
...
class DHDT(tf.Module):
    ...
    def forward_old(self, X):
        ...
        split_index_array_complete = split_index_array_complete - tf.stop_gradient(split_index_array_complete - tfa.seq2seq.hardmax(split_index_array_complete))
        ...
        for leaf_index, path in enumerate(paths):
            ...
            for internal_node_index in path: 
                ...
                split_decision = (respective_input_value - internal_node_split_value) - tf.stop_gradient((respective_input_value - internal_node_split_value) - tf.cast(tf.greater(respective_input_value, internal_node_split_value), tf.float32))
    ...
    def forward(self, X):
        ...
        #internal_node_result_complete = tf.sigmoid(self.beta_2 * (X_by_index - split_values_by_index)) - tf.stop_gradient(internal_node_result_complete - tf.round(tf.sigmoid(1000 * (X_by_index - split_values_by_index))))
        
        #internal_node_result_complete = (X_by_index - split_values_by_index)
        internal_node_result_complete = sigmoid(X_by_index - split_values_by_index) - tf.stop_gradient(sigmoid(X_by_index - split_values_by_index) - tf.cast(tf.greater(X_by_index, split_values_by_index), tf.float32))
        
        #internal_node_result_complete = tf.stop_gradient(tf.cast(tf.greater(X_by_index, split_values_by_index), tf.float32))
        ...
        #internal_node_result_complete = internal_node_result_complete + (1/np.pi)*tf.reduce_sum([(-1)**n*tf.math.sin(2*np.pi*n*internal_node_result_complete)/n for n in range(1, 10)], axis=0)
