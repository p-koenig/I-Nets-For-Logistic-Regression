...
from utilities.utilities_updated13 import *
from utilities.DHDT_updated13 import *
...
class DHDT(tf.Module):
    ...
    def forward(self, X):
        ...
        split_index_array_complete = tfa.activations.sparsemax(self.split_index_array) - tf.stop_gradient(tfa.activations.sparsemax(self.split_index_array) -  tfa.seq2seq.hardmax(self.split_index_array))#tf.cast(tf.cast(split_index_array_complete, tf.int64), tf.float32)
        ...
        internal_node_result_complete = tf.sigmoid(X_by_index - split_values_by_index) - tf.stop_gradient(tf.sigmoid(X_by_index - split_values_by_index) - tf.cast(tf.greater(X_by_index, split_values_by_index), tf.float32))
