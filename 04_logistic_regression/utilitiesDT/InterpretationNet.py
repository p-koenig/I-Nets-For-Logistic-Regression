...
def interpretation_net_training(lambda_net_train_dataset, 
    ...
    if config['computation']['load_model']:
        print("#################################################### ERROR LOAD MODEL NOT IMPLEMENTED ####################################################")
        # paths_dict = generate_paths(config, path_type = 'interpretation_net')
# 
        # path = './dataDT/results/' + paths_dict['path_identifier_interpretation_net'] + '/history' + '.pkl'
...
def load_inet(loss_function, metrics, config):
    ...
    from utilitiesDT.utility_functions import generate_paths
   ...
   # if config['i_net']['nas']:
   #     path = inet_path_DT(config) + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + dt_string         
   #     #path = inet_path_DT(config) + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + '_' + paths_dict['path_identifier_lambda_net_data'] + dt_string         
   # else:
   #     path = inet_path_DT(config) + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version'])
   #     
   #     #path = inet_path_DT(config) + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version'])
#
    path = inet_path_DT(config) '/INetSize5000_numLNets1_var15_class2_distribution_xMax1_xMin0_xDistuniform_dNoise0_randParamDist_maxDistClass1_distribParamMax5_randClassProb_exLinSepun-no-ga-be-po_depth3_beta1_decisionSpars1_vanilla_fullyGrown/128_e1000ES0.001_b64_drop0_adam_binary_crossentropy_fixedInit1-seed42/inet_dense1792-512-512_drop0-0-0.5e1b256_adam_funcRep5_reshapeNone_depth3_beta1_decisionSpars1_vanilla_reshapeNone'
...
def load_encoder_model(config): 
    ...
    paths_dict = generate_paths(config, path_type = 'interpretation_net')
    if config['i_net']['nas']:
        path = inet_path_DT(config) + 'encoder_model_' + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + dt_string         
        #path = inet_path_DT(config) + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + '_' + paths_dict['path_identifier_lambda_net_data'] + dt_string         
    else:
        path = inet_path_DT(config) + 'encoder_model_' + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version'])
...
def generate_inet_labels_learned(lambda_net, config, config_adjusted):
    ...
    from utilitiesDT.utility_functions import network_parameters_to_network, get_shaped_parameters_for_decision_tree, get_parameters_from_sklearn_decision_tree
...
def train_inet(lambda_net_train_dataset,
    ...
    from utilitiesDT.utility_functions import unstack_array_to_list    
    ...
    if config['i_net']['separate_weight_bias']:
        ...
        lambda_network_layers_complete = flatten_list([config['data']['n_features'], config['lambda_net']['lambda_network_layers']])
    ...
    if use_distribution_list and config['data']['max_distributions_per_class'] is not None:
        ...
        if True:
            ...
            random_evaluation_dataset_flat_array_train = random_evaluation_dataset_array_train.reshape((-1, random_evaluation_dataset_array_train.shape[1]*config['data']['n_features']))                        
            random_evaluation_dataset_flat_array_valid = random_evaluation_dataset_array_valid.reshape((-1, random_evaluation_dataset_array_valid.shape[1]*config['data']['n_features']))
            ...
            random_evaluation_dataset_flat_array_train = random_evaluation_dataset_array_train.reshape((-1, config['evaluation']['random_evaluation_dataset_size']*config['data']['n_features']))  
            ...
            random_evaluation_dataset_flat_array_valid = random_evaluation_dataset_array_valid.reshape((-1, config['evaluation']['random_evaluation_dataset_size']*config['data']['n_features']))   
    ...
    if not config['computation']['load_model']:              
        if config['i_net']['nas']:
            ...
            with CustomObjectScope(custom_object_dict):
                ...
                if config['i_net']['function_representation_type'] == 1:
                    if config['function_family']['dt_type'] == 'SDT':     
                        if config['i_net']['additional_hidden']:
                            ...
                            outputs_coeff = CustomDenseInet(internal_node_num_ * config['data']['n_features'], 
                        ...
                        for outputs_index in range(internal_node_num_):
                            for var_index in range(config['function_family']['decision_sparsity']):
                                if config['i_net']['additional_hidden']:
                                    ...
                                    outputs_identifer = CustomDenseInet(neurons=config['data']['n_features'], 
                                                                        activation='softmax', 
                    ...
                    if config['function_family']['dt_type'] == 'SDT':                              
                        ...
                        for outputs_index in range(internal_node_num_):
                            if config['i_net']['additional_hidden']:
                                hidden_node_outputs_identifer = SingleDenseLayerBlock()(hidden_node)
                                outputs_identifer = CustomDenseInet(config['data']['n_features'], 
                                                                    activation='softmax', 
                                                                    seed=config['computation']['RANDOM_SEED'])(hidden_node_outputs_identifer)
                            else:                             
                                outputs_identifer = CustomDenseInet(config['data']['n_features'], 
                        ...
                        if config['i_net']['additional_hidden']:
                            hidden_node_outputs_coeff = SingleDenseLayerBlock()(hidden_node)
                            outputs_coeff = CustomDenseInet(neurons=internal_node_num_*config['data']['n_features'], 
                                                            activation='sigmoid', 
                                                            seed=config['computation']['RANDOM_SEED'])(hidden_node_outputs_coeff)
                        else:                              
                            outputs_coeff = CustomDenseInet(neurons=internal_node_num_*config['data']['n_features'], 
                ...
                directory = './dataDT/autokeras/' + paths_dict['path_identifier_lambda_net_data'] + '/' + paths_dict['path_identifier_lambda_net_data'] + dt_string + '/' + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_reshape' + str(config['i_net']['data_reshape_version']) + '_' + timestr
                #directory = './dataDT/autokeras/' + paths_dict['path_identifier_lambda_net_data'] + dt_string + '/' + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_reshape' + str(config['i_net']['data_reshape_version']) + '_' + timestr
                ...
                model.save(inet_path_DT(config) + config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + dt_string , save_format='tf')   
                
                if encoder_model is not None:
                    encoder_model.save(inet_path_DT(config) + 'encoder_model_' +  config['i_net']['nas_type'] + '_' + str(config['i_net']['nas_trials']) + '_' + str(config['i_net']['data_reshape_version']) + dt_string , save_format='tf')
            ...
            if config['i_net']['function_representation_type'] == 1:
                if config['function_family']['dt_type'] == 'SDT':
                    outputs_coeff_neurons = internal_node_num_ * config['data']['n_features']
                    if config['i_net']['additional_hidden']:
                ...
                if config['function_family']['dt_type'] == 'SDT':                        
                    ...
                    for outputs_index in range(internal_node_num_):
                        for var_index in range(config['function_family']['decision_sparsity']):
                            output_name = 'output_identifier' + str(outputs_index+1) + '_var' + str(var_index+1) + '_' + str(config['function_family']['decision_sparsity'])
                            outputs_identifer_neurons = config['data']['n_features']
                            if config['i_net']['additional_hidden']:
            ...
            model.save(inet_path_DT(config) + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version']), save_format='tf')
            if encoder_model is not None:
                encoder_model.save(inet_path_DT(config) + 'encoder_model_' + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version']), save_format='tf')        

            
            #model.save(inet_path_DT(config)  + '_' + paths_dict['path_identifier_interpretation_net'] + dt_string + '_reshape' + str(config['i_net']['data_reshape_version']), save_format='tf')
...
def normalize_lambda_net(flat_weights, random_evaluation_dataset, base_model=None, config=None): 
    ...
    from utilitiesDT.LambdaNet import weights_to_model
...
def restructure_network_parameters(shaped_network_parameters, config):
    ...
    if config['i_net']['data_reshape_version'] == 0: #one sequence for biases and one sequence for weights per layer (padded to maximum size)
        ...
        lambda_net_structure = flatten_list([config['data']['n_features'], config['lambda_net']['lambda_network_layers'], 1 if config['data']['num_classes'] == 2 else None])                 
...
def autoencode_data(X_data, config, encoder_model=None):
    ...
    if encoder_model is None:
        ...
        encoder_model = AutoEncoders(num_features=X_data.shape[1], reduction_size=2*config['data']['n_features']*(2**config['function_family']['maximum_depth'])) #5
...
def restructure_data_cnn_lstm(X_data, config, subsequences=None):
    ...
    from utilitiesDT.utility_functions import generate_base_model, shape_flat_network_parameters
...
def generate_history_plots(history, config):
    ...
    plt.savefig(inet_path_DT(config) + "results" + paths_dict['path_identifier_interpretation_net'] + '/' + list(history.keys())[len(history.keys())//2+1] + '.png')
    plt.clf()
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig(inet_path_DT(config) + "results" + paths_dict['path_identifier_interpretation_net'] + '/loss_' + '.png')   
...
def save_results(history, config):
    ...
    path = inet_path_DT(config) + "results"  + paths_dict['path_identifier_interpretation_net'] + '/history' + '.pkl'
