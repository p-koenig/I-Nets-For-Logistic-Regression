...
def inet_decision_function_fv_loss_wrapper_parameters(config):   
    ...
    def inet_decision_function_fv_loss_parameters(function_true_with_network_parameters, function_pred):     
        ...
        config['function_family']['basic_function_representation_length'] = internal_node_num_ * config['data']['n_features'] * 2 + leaf_node_num_ * config['data']['num_classes']
        ...
        splits_features_true = tf.reshape(decision_tree_parameters[:,:internal_node_num_ * config['data']['n_features']], shape=(-1, internal_node_num_, config['data']['n_features']))
        splits_values_true = tf.reshape(decision_tree_parameters[:,internal_node_num_* config['data']['n_features']:internal_node_num_ * config['data']['n_features'] * 2], shape=(-1, internal_node_num_, config['data']['n_features']))
        leaf_probabilities_true = tf.reshape(decision_tree_parameters[:,internal_node_num_ * config['data']['n_features'] * 2:], shape=(-1, leaf_node_num_, config['data']['num_classes']))[:,:,0]

        splits_features_pred = tf.reshape(function_pred[:,:internal_node_num_ * config['data']['n_features']], shape=(-1, internal_node_num_, config['data']['n_features']))
        splits_values_pred = tf.reshape(function_pred[:,internal_node_num_* config['data']['n_features']:internal_node_num_ * config['data']['n_features'] * 2], shape=(-1, internal_node_num_, config['data']['n_features']))

        leaf_probabilities_pred = tf.reshape(function_pred[:,internal_node_num_ * config['data']['n_features'] * 2:internal_node_num_ * config['data']['n_features'] * 2 + leaf_node_num_], shape=(-1, leaf_node_num_))
...
def inet_decision_function_fv_loss_wrapper(model_lambda_placeholder, network_parameters_structure, config, use_distribution_list):   
    ...
    def inet_decision_function_fv_loss(function_true_with_network_parameters, function_pred):      
        ...
        #tf.print("internal_node_num_ * config['data']['n_features'] * 2 + leaf_node_num_ * config['data']['num_classes']", internal_node_num_ * config['data']['n_features'] * 2 + leaf_node_num_ * config['data']['num_classes'])
...
def calculate_function_values_loss_decision_wrapper(network_parameters_structure, model_lambda_placeholder, config, use_distribution_list):
    ...
    def calculate_function_values_loss_decision(input_list):  
        ...
        random_evaluation_dataset = generate_random_data_points_custom(config['data']['x_min'], config['data']['x_max'], config['evaluation']['random_evaluation_dataset_size'], config['data']['n_features'], categorical_indices=None, distrib=config['evaluation']['random_evaluation_dataset_distribution'])
            
        #else:
        #    pass
        #    # random_evaluation_dataset = tf.reshape(tensor=distribution_line, shape=(-1,config['data']['n_features']))
...
def calculate_function_value_from_decision_tree_parameters_wrapper(random_evaluation_dataset, config):
    ...
    def calculate_function_value_from_decision_tree_parameters(function_array):    
        ...
        from utilitiesDT.utility_functions import get_shaped_parameters_for_decision_tree      
...
def calculate_function_value_from_vanilla_decision_tree_parameters_wrapper(random_evaluation_dataset, config):
    ...
    def calculate_function_value_from_vanilla_decision_tree_parameters(function_array):
        ...
        function_values_vanilla_dt = tf.vectorized_map(calculate_function_value_from_vanilla_decision_tree_parameter_single_sample_wrapper(weights, leaf_probabilities, leaf_node_num_, internal_node_num_, maximum_depth, config['data']['n_features']), random_evaluation_dataset)
...
def calculate_function_value_from_vanilla_decision_tree_parameter_single_sample_wrapper(weights, leaf_probabilities, leaf_node_num_, internal_node_num_, maximum_depth, n_features):
...
def inet_decision_function_fv_metric_wrapper(model_lambda_placeholder, network_parameters_structure, config, metric, use_distribution_list):
    ...
    def inet_decision_function_fv_metric(function_true_with_network_parameters, function_pred):    
        ...
        #random_evaluation_dataset = generate_random_data_points_custom(config['data']['x_min'], config['data']['x_max'], config['evaluation']['random_evaluation_dataset_size'], config['data']['n_features'], categorical_indices=None, distrib=config['evaluation']['random_evaluation_dataset_distribution'])            
        #random_evaluation_dataset =  np.random.uniform(low=config['data']['x_min'], high=config['data']['x_max'], size=(config['evaluation']['random_evaluation_dataset_size'], config['data']['n_features']))
        #random_evaluation_dataset = tf.dtypes.cast(tf.convert_to_tensor(random_evaluation_dataset), tf.float32)
        if not config['i_net']['function_value_loss']:
            internal_node_num_ = 2 ** config['function_family']['maximum_depth'] - 1 
            leaf_node_num_ = 2 ** config['function_family']['maximum_depth'] 
...
def root_mean_squared_error(y_true, y_pred):   
    ...
    from utilitiesDT.utility_functions import return_numpy_representation
