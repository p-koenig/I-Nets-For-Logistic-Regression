...
def train_inet(lambda_net_train_dataset,
    ...
    if not config['computation']['load_model']:              
        if config['i_net']['nas']:
            ...
            elif config['i_net']['function_representation_type'] >= 4:                
                if config['function_family']['dt_type'] == 'SDT':
                    ...
                    if config['i_net']['additional_hidden']:
                        ...
                        activation_special = None
                        if config['i_net']['function_representation_type'] == 4:  
                            activation_special = linear_clip
                        if config['i_net']['function_representation_type'] == 5:  
                            activation_special = scaled_softmax_double                            
                        if config['i_net']['function_representation_type'] == 6:  
                            activation_special = scaled_softmax_half
                            
                        from keras.utils.generic_utils import get_custom_objects

                        get_custom_objects().update({'activation_special': Activation(activation_special)})                            
