...
class LambdaNet():
    ...
    def __init__(self, line_weights, line_distribution_parameters, X_data, y_data, config):
        from utilitiesDT.utility_functions import network_parameters_to_network, generate_decision_tree_from_array, get_number_of_function_parameters, shaped_network_parameters_to_array, normal_neural_net, shape_flat_network_parameters, generate_base_model
        
        assert isinstance(line_weights, np.ndarray), 'line is no array: ' + str(line_weights) 
        
        self.n_features = config['data']['n_features']  
        self.number_of_classes = config['data']['n_targets']  
        
        self.function_parameter_size = get_number_of_function_parameters(config['function_family']['dt_type'], config['function_family']['maximum_depth'], self.n_features, self.number_of_classes)#(2 ** config['function_family']['maximum_depth'] - 1) * (self.n_features + 1) + (2 ** config['function_family']['maximum_depth']) * self.number_of_classes
        
        #self.network_parameter_size = get_number_of_lambda_net_parameters(config['lambda_net']['lambda_network_layers'], self.n_features, self.number_of_classes)
        ...
        #self.X_test_lambda = np.transpose(np.array([line_X_data[i::self.n_features] for i in range(self.n_features)]))
            ...
            if line_distribution_parameters[0] is None:
                ...
                if 'make_class' in config['data']['function_generation_type']:
                    ...
                    if distributions_per_class == 0:
                        ...
                        distribution_list = line_distribution_parameters.reshape(-1, 1+distributions_per_class*config['data']['n_targets']*2)
    ...
    def return_column_names(self, config):  
        ...
        from utilitiesDT.utility_functions import flatten_list, generate_decision_tree_identifier
...
def generate_lambda_net_from_config(config, seed=None):
    ...
    from utilitiesDT.utility_functions import generate_base_model
...
def train_lambda_net(config,
    ...
    from utilitiesDT.utility_functions import pairwise, shaped_network_parameters_to_array
    from utilitiesDT.LambdaNet import generate_lambda_net_from_config
    
    #assert (X_data_with_function[0] == y_data_with_function[0]).all()
...
#def get_number_of_lambda_net_parameters(lambda_network_layer_list, n_features, n_targets):
def get_number_of_lambda_net_parameters(config):
    
    from utilitiesDT.utility_functions import generate_base_model, shaped_network_parameters_to_array
    ...
    if False:
        layers_with_input_output = flatten_list([n_features, lambda_network_layer_list, [1 if n_targets == 2 else n_targets]])
