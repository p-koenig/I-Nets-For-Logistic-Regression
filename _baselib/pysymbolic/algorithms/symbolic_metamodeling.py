...
def tune_single_dim(lr, n_iter, x, y, verbosity=False):
    ...
    for u in range(n_iter):
        ...
        a         = min(a - lr * min(grads_a, a), 3)
        b         = min(b - lr * min(grads_b, b), 3)
        c         = min(c - lr * min(grads_c, c), 3)
...
class symbolic_metamodel:
    ...
    def symbolic_expression(self):
        ...
        if self.num_basis != self.init_model.coef_.shape[0]:
            self.init_model.coef_ = self.init_model.coef_.reshape(self.num_basis,)
        ...
        for v in range(self.num_basis):
            ...
            print(self.init_model.coef_[v] * re(f_curr.expression()))
